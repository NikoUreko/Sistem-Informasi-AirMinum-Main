...
@section('title')
Tambah Villa | Chillin'Vibe Admin
@endsection

@section('content')
<h3>Input Data Villa</h3>
<div class="form-login">
  <form action="{{ url('/villa/store') }}" method="post" enctype="multipart/form-data">
    @csrf
    ...
    <label for="pemakaian_level_1">Pemakaian (0-10m³)</label>
    <input class="input-readonly" type="number" name="pemakaian_level_1" id="pemakaian_level_1" placeholder="0-10m³" value="{{ old('pemakaian_level_1') }}" readonly />
    <p class="tarif-label">Tarif I: Rp <span id="tarif_1_display" class="tarif-value">0</span></p>
    @error('pemakaian_level_1')
    <p style="font-size: 10px; color: red">{{ $message }}</p>
    @enderror

    <label for="pemakaian_level_2">Pemakaian (11-30m³)</label>
    <input class="input-readonly" type="number" name="pemakaian_level_2" id="pemakaian_level_2" placeholder="11-30m³" value="{{ old('pemakaian_level_2') }}" readonly />
    <p class="tarif-label">Tarif II: Rp <span id="tarif_2_display" class="tarif-value">0</span></p>
    @error('pemakaian_level_2')
    <p style="font-size: 10px; color: red">{{ $message }}</p>
    @enderror

    <label for="pemakaian_level_3">Pemakaian (>31m³)</label>
    <input class="input-readonly" type="number" name="pemakaian_level_3" id="pemakaian_level_3" placeholder=">31m³" value="{{ old('pemakaian_level_3') }}" readonly />
...
document.addEventListener("DOMContentLoaded", function () {
      ...
      function calculatePemakaian() {
        ...
        pemakaianLevel1Input.value = Math.min(sisaPemakaian, 10);
        sisaPemakaian = Math.max(0, sisaPemakaian - 10);

        pemakaianLevel2Input.value = Math.min(sisaPemakaian, 10);
        sisaPemakaian = Math.max(0, sisaPemakaian - 10);
