...
@section('title')
Edit Kos | Chillin'Vibe Admin
@endsection

@section('content')
<h3>Edit Data Kos</h3>
<div class="form-login">
  <form action="{{ url('/kos/update/' . $kos->id) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('put')
    ...
    <input class="input" type="text" name="no_meterblnini" id="no_meterblnini" placeholder="no meter bulan ini"
      value="{{ old('no_meterblnini', $kos->no_meterblnini) }}" />
    ...
    <input class="input" type="text" name="no_meterblnlalu" id="no_meterblnlalu" placeholder="no meter bulan lalu"
      value="{{ old('no_meterblnlalu', $kos->no_meterblnlalu) }}" />
    ...
    <input class="input-readonly" type="text" name="pemakaian_M" id="pemakaian_M" placeholder="pemakaian M"
      value="{{ old('pemakaian_M', $kos->pemakaian_M) }}" readonly/>
    @error('pemakaian_M')
    <p style="font-size: 10px; color: red">{{ $message }}</p>
    @enderror

    <label for="pemakaian_1">Pemakaian (0-20m³)</label>
    <input class="input-readonly" type="number" name="pemakaian_1" id="pemakaian_1" placeholder="0-20m³"
      value="{{ old('pemakaian_1', $kos->pemakaian_1) }}" readonly />
      <p class="tarif-label">Tarif I: Rp <span id="tarif_1_display" class="tarif-value">0</span></p>
    @error('pemakaian_1')
    <p style="font-size: 10px; color: red">{{ $message }}</p>
    @enderror

    <label for="pemakaian_2">Pemakaian (21-40m³)</label>
    <input class="input-readonly" type="number" name="pemakaian_2" id="pemakaian_2" placeholder="21-40m³"
      value="{{ old('pemakaian_2', $kos->pemakaian_2) }}" readonly />
      <p class="tarif-label">Tarif II: Rp <span id="tarif_2_display" class="tarif-value">0</span></p>
    @error('pemakaian_2')
    <p style="font-size: 10px; color: red">{{ $message }}</p>
    @enderror

    <label for="pemakaian_3">Pemakaian (>41m³)</label>
    <input class="input-readonly" type="number" name="pemakaian_3" id="pemakaian_3" placeholder=">41m³"
      value="{{ old('pemakaian_3', $kos->pemakaian_3) }}" readonly />
...
document.addEventListener("DOMContentLoaded", function () {
      ...
      function calculatePemakaian() {
        ...
        pemakaianLevel1Input.value = Math.min(sisaPemakaian, 20);
        sisaPemakaian = Math.max(0, sisaPemakaian - 20);

        pemakaianLevel2Input.value = Math.min(sisaPemakaian, 20);
        sisaPemakaian = Math.max(0, sisaPemakaian - 20);
